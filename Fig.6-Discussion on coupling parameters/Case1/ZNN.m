function ZNN

global  ZJX  ZJXX  eZNN 
ZJX=4*ones(4,1);  
ZJXX=4; 
tspan=0:0.01:5; 
%If gamma is 1, then the end-time is 10s for creating figures
%If gamma is 10,then the end-time is 1s  for creating figures
%If gamma is 100,then the end-time is 0.1s for creating figures
options=odeset( 'MaxStep',0.01,'RelTol',1e-4,'AbsTol',1e-8*ones(7,1)); 
gamma=0;

    y0=[zeros(1);zeros(4,1)]; 
    %x0=3*(2*rand(2,1)-ones(2,1));
    x0=[2.6;2.6];
  init=[x0;y0];
    [t,x]=ode15s(@ZNN_con_righthandside, tspan, init, options, gamma);
         total=length(t);
     for i=1:total
         error(i)=norm(eZNN{i},2);
     end

    figure(3)
    plot(t,error,'k-.','linewidth',3,'color','#96cccb');
    xlabel('t(s)');
    hold on;
    set(gca,'FontName','Times New Roman','FontSize',16,'LineWidth',1.5);
    
    
%      figure(3)
%     subplot(2,1,1);
%     plot(t,x(:,1),'c--','linewidth',3);  
%     xlabel('t(s)');
%     ylabel('x_1(t)');
%     legend('Our','VPRNN');
%     yline(ZJXX,'--b','linewidth',1.5);
%     hold on 
%     yline(-ZJXX,'--b','linewidth',1.5);
%     %legend('x_1(t)');
%    set(gca,'FontName','Times New Roman','FontSize',16,'LineWidth',1.5);
%    % subplot(2,1,2);plot(t,x2,'b-.','linewidth',1.5);hold on
%      subplot(2,1,2);plot(t,x(:,2),'c--','linewidth',2.5); 
%      legend('Our','VPRNN');
%     yline(ZJXX,'--b','linewidth',1.5);
%     hold on 
%     yline(-ZJXX,'--b','linewidth',1.5);
%      %legend('x_2(t)');
%      xlabel('t(s)');
%        ylabel('x_2(t)');
%      hold on
%       legend('Our','VPRNN');
%    set(gca,'FontName','Times New Roman','FontSize',16,'LineWidth',1.5);
%  
% plot(t,epos3(:,1),'m--',t,epos3(:,2),'b-',t,epos3(:,3),'k-.','linewidth',2.5);hold on;
% xlabel('Time (Second)');
% x=[1.73 6.74 6.74 1.73];
% y=[-0.5 -0.5 0.5 0.5];
% fill(x,y,'r');
% alpha(0.1)
% title('Cooperative position error3');
% legend('ex','ey','ez');
